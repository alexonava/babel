import { AdditiveBlending, BackSide, BoxGeometry, BufferAttribute, BufferGeometry, CanvasTexture, CircleGeometry, Color, CylinderGeometry, DodecahedronGeometry, DoubleSide, Float32BufferAttribute, Group, IcosahedronGeometry, LinearFilter, LinearMipmapLinearFilter, Mesh, MeshBasicMaterial, MeshStandardMaterial, OctahedronGeometry, PlaneGeometry, PointLight, Points, PointsMaterial, RepeatWrapping, RingGeometry, Sprite, SpriteMaterial, TetrahedronGeometry, TorusGeometry } from "three";
import { createBrickDetailController } from "./brick-detail.js";
import { createLegacyReflection } from "./legacy-reflection.js";

// Preserved procedural construction for comparison URLs, low quality, and
// authored-asset failures. Keep placement/material constants stable here.
export function createLegacyWorld({
  THREE,
  TOWER_SURFACE_MATERIALS,
  WORLD,
  atmosphereSystem,
  chooseAnisotropy,
  circleGeometry,
  clamp01,
  cloudAnchor,
  createGroundOverlayTexture,
  createMarbleMaterial,
  createMarbleTextures,
  createTowerTextures,
  environmentSystem,
  filmEffects,
  filmEnabled,
  groundHeight,
  environmentRoot: group4,
  towerRoot: group7,
  homeScene,
  invalidate,
  overlaySegments,
  ...
  renderer,
  rendering,
  towerGroundY: result107,
  collapseYaw: num511,
  setShadowParticipation,
  setSrgbTexture,
  smoothstep01,
  state,
  subsystemRegistry,
  towerSystem,
  upperGlowSpriteCount,
  architectureEnabled,
  classicTowerMeshes,
  baseMasonryRecords,
  getProfileCount,
  ...
  cloudViewFade,
    ...
    let classicTree = null, groundOverlay = null;
    ...
    position9.needsUpdate = true;
    circleGeometry.computeVertexNormals();
    group14.children.forEach(object => {
      object.position.y = groundHeight(object.position.x, object.position.z) - 0.12;
    });
    plantShadowRecords.forEach(object => {
      object.position.y = groundHeight(object.position.x, object.position.z) - 0.1;
    });
    subsystemRegistry.register(createLegacyReflection({
      enabled: !state.lowPower && (!filmEnabled || !quietSetting),
      homeScene, renderer, rendering, puddles: group14, groundY: groundHeight(0, 0),
    }));
    ...
    return {
      classicTree, groundOverlay,
      effects: [mesh41, ...arr24, ...arr22.map(record => record.mesh), crownEffectLight],
      plinth: [ftLower, mesh37, ftSeam],
      footing: [ftLower, mesh37, ftSeam, ftSoot],
      base: mesh38,
      rubble: group12.children.filter(object => object.isMesh),
      nearbyStones: group5.children.filter(object => object.isMesh && Math.hypot(object.position.x, object.position.z) <= 28),
      stones: group5.children.filter(object => object.isMesh),
      plants: group6.children.filter(object => object.isGroup && object !== classicTree),
      plantRecords: arr19, torches: arr23, trim: [ftSeam],
      clouds: [...driftClouds.map(c => c.mesh), ...arr16.map(c => c.mesh), ...arr17.map(c => c.mesh),
        ...emberClouds.map(c => c.mesh), ...hazeClouds.map(c => c.mesh), ...pulseClouds.flatMap(c => c.puffs.map(p => p.sprite))],
      update({ elapsedTime, quiet, grounded, visibilityScale }) {
        const tmpV55 = 1;
        (
          ...
          ? (() => {
              ...
              driftClouds.forEach((arg6, arg7) => {
                ...
                arg6.mesh.material.opacity =
                  (state.lowPower ? 0.14 : 0.19) * visibilityScale +
              ...
              emberClouds.forEach((arg12, arg13) => {
                ...
                ((arg12.mesh.position.x = Math.cos(num173) * num174 + arg12.drift),
                  ...
                  (arg12.mesh.material.opacity =
                    (state.lowPower ? 0.08 : 0.11) * visibilityScale +
                    0.03 +
          ...
          ? (() => {
              ...
              hazeClouds.forEach((arg16, arg17) => {
                ...
                arg16.mesh.material.opacity =
                  (state.lowPower ? 0.08 : 0.12) * visibilityScale + 0.02 * Math.sin(0.5 * num179);
              });
            })()
