...
- The camera tour has no shot label or control strip. Its only control is a quiet Pause scene / Play scene text button in the footer (the label names the next action): meta grey, 12px, with at least a 44px target. Preserve the per-shot holds (The watch and Portrait 9 seconds, Threshold and Gallery detail 7, Lantern study, Close-up and Root and lantern 6), the 1-second dissolve staggered by depth (sky, then mountains, ground and subject) with no dip to black, drift that never stops before a cut, and reduced-motion/panel pauses; fixed review cadences remain available through URL parameters.
