...
## Estate homepage delivery

The homepage opens directly on the illustrated estate. Profile, Experience, and Contact use accessible parchment dialogs, with equivalent inline content when JavaScript is disabled. The app never imports the scene runtime, even with old scene query parameters. Verify no scene bundle, model, or scene-texture requests on desktop and phone. Estate artwork has transparent edges and content-hashed URLs; shared paper assets load independently of the removed scene.

Production smoke checks require estate navigation and exactly one hashed app and CSS reference. Only rollback verification accepts the previous scene reference. Asset parity, canonical hostname, security headers, redirects, 404 checks, and automatic rollback remain mandatory.

## Archived scene delivery policy

The following scene and pilot notes describe preserved rollback source; their query parameters do not activate the current homepage.
...
## Classic construction pilot validation
